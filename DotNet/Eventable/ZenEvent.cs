...
namespace ZenEvent
{
    ...
    */
    public class ZenEvent : IZenEvent, IZenNodePreInit, IZenNodeInit
    {
        ...
        *
        * For example, elements with id's "ButtonOn" and "ButtonOff" that are defined in Zenodys Visual Development Tool,
        * share same implementation - ZenButton.
        *
        * This event is fired for all elements that share same implementation. Here we inform Zenodys Computing Engine
        * that event occured. If we pass element Id in ModuleEventData instance, then Computing Engine
        * automatically filters elements and CheckInterruptCondition callback is called only for element with matching Id.
        ...
        */
        ...
        /**
        * Callback that is called from Computing Engine. If element Id is specified in ModuleEventData instance
        * then only element with matching id is called.
        *
        * @param    eventData : information passed from original event handler (third parameter of ModuleEventData instance)
        * @param    element : contains all necessary information about current element (property values, last executed time, last error....)
        * @param    elements : all elements. Useful for getting results, errors, states... from other elements or executing them dynamically.
        *
        * @return   bool    if true then computing Engine resumes loop to element that proceeds current, otherwise loop is not resumed
        */
